...
import { postManifest } from '@/api/manifests';
...
getImages((res) => { images.value = res.data; if (res.data.length > 0) { image.value = res.data[0].Name } }, (err) => { console.log(err) })

const name = ref('sample_manifest')
const image = ref('')
const tag = ref('')
const restart = ref('')
const retry = ref(0)
...
const save = function() {
  ...
  postManifest(data, (res) => { notify({text: 'manifest created'}); router.push({ name: 'manifests' }) }, (err) => { notify({type: 'error', text: err.response.data.message }) })
}

const loadTextFromFile = function(ev) {
      const file = ev.target.files[0];
      const reader = new FileReader();
      reader.onload = function(e) {
        try {
          const importer = JSON.parse(e.target.result)
          const importer_str = JSON.parse(importer.structure)
          name.value = importer.title
          image.value = importer_str.image
          tag.value = importer_str.tag
          restart.value = importer_str.restart
          retry.value = importer_str.retry
          timeout.value = importer_str.timeout
          network.value = importer_str.network
          envs.value = importer_str.envs
          labels.value = importer_str.labels
          ports.value = importer_str.ports
          binds.value = importer_str.binds
        } catch (error) {
          alert(error)
        }
      }
